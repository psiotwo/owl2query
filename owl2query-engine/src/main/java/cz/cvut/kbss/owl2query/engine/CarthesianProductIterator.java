...
class CarthesianProductIterator<G> implements Iterator<ResultBinding<G>> {
	...
	private final List<Iterator<ResultBinding<G>>> iterators = new ArrayList<>();
	private final List<ResultBinding<G>> cursor = new ArrayList<>();
	...
	public ResultBinding<G> next() {
		...
		ResultBinding<G> result = new ResultBindingImpl<>();

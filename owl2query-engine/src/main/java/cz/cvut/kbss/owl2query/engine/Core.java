...
class Core<G> implements QueryAtom<G> {
	...
	private final Term<G> term;

	private final GroundTerm<G> rollUp;
	...
	public QueryAtom<G> apply(
		...
		if (binding.containsKey(term)) {
			return new Core<>(binding.get(term), rollUp, query.apply(binding));
		} else {
			return new Core<>(term, rollUp, query.apply(binding));
		}
	}
